...
export async function updateTask(
  ...
  const [{ data: previousTask }, { count: activeCount }] = await Promise.all([
    supabase
      .from("tasks")
      .select("status")
    ...
    parsed.data.assignee_id
      ? supabase
          ...
          .eq("assignee_id", parsed.data.assignee_id)
  ...
  return { success: true, task: updatedTask, warning: activeWarning };
}
