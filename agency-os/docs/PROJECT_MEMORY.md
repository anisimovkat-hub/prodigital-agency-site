...
- Следующее для Codex: фильтр на «Неделя» (по образцу «Сегодня»); UI управления `recurring_tasks`
  (+ добавить их тип в `lib/supabase/types.ts`); автопостинг каруселей — внешний пайплайн, не CRM.
