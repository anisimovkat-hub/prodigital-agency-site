...
- **Фильтр на «Неделя»** по категориям Все/Мои/Личные/Команда/сотрудник (сейчас реализован
  только на «Сегодня»; повторить тот же паттерн из `today/page.tsx`).
- **UI управления шаблонами повторов** (`recurring_tasks`): страница/секция для создания,
  включения/выключения (`is_active`) и смены частоты — сейчас шаблоны заводятся только SQL-ом.
  Типы для `recurring_tasks` в `lib/supabase/types.ts` пока НЕ добавлены (добавить приUI).
