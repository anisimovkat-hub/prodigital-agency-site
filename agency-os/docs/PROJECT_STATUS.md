...
### Ожидает применения (нужен Supabase MCP — в момент работы был отключён)
- **Миграция `0006_recurring_tasks.sql`** создана и закоммичена, но НЕ применена к БД.
  Применить: `apply_migration` (таблица `recurring_tasks`, колонка `tasks.recurring_task_id`,
  функция `generate_recurring_tasks(date)`, ежедневный pg_cron `generate-recurring-tasks` в 03:00 UTC).
- **Блог-проект и шаблоны повторов** (по решению владельца): создать клиента «Личный бренд
  (Катерина)» + проект «Личный бренд» (responsible=Катерина) с направлениями
  Контент/Постинг/Автоматизация; завести шаблоны в `recurring_tasks`: «Выложить stories» (daily),
  «Опубликовать reels» (every_other_day), «Сделать карусель» (daily) — assignee=Катерина,
  workstream=Постинг; затем `SELECT generate_recurring_tasks();` для генерации на сегодня.
- **Фильтр на «Неделя»** по тем же категориям (пока только на «Сегодня»).
- Управление шаблонами повторов из UI и «нейронка-автопостинг каруселей» (внешний пайплайн) —
  отдельные follow-up; в CRM это регулярная задача + место под готовый файл, не сам автопостинг.
