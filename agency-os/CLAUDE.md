...
**Продакшн на 2026-07-25:** миграции 0001–0006 применены, роли/RLS, инвайты, автоматическое
удаление старых выполненных и генерация повторяющихся задач работают. Production собирается из ветки
`claude/agency-ops-mvp-design-ykrrn0`. Страницы «Проекты», «Задачи», «Доска», «Неделя» и
дашборд работают на реальных данных. Пять сотрудников ранее получили временные пароли;
...
- Production env в Vercel: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` и
  защищённый `GOOGLE_CALENDAR_ICAL_URL` для read-only личного календаря владельца
...
│   ├── 0004_task_retention.sql     # дата выполнения + очистка через pg_cron
...
- **Личный Google Calendar владельца** подключён через закрытый read-only iCal URL, который
  хранится только как sensitive production env `GOOGLE_CALENDAR_ICAL_URL`. Server-only слой
  загружает календарь с кэшем 5 минут, корректно разворачивает RRULE/EXDATE/исключения и
  отображает события в часовом поясе календаря (`Asia/Bangkok`). События видит только роль
...
- **Фильтр аудитории задач** (`lib/task-audience-filter.ts`) общий для «Сегодня» и «Недели»:
  Все / Мои / Личные / Команда / конкретный сотрудник. Сначала база ограничивает доступ через RLS,
  затем представление фильтрует уже разрешённые строки.
- **Дашборд-пульт**: секции «Мой день» (задачи текущего пользователя на сегодня+просрочка),
  ...
  server-компоненте из `tasks`/`profiles`; RLS отдаёт владельцу все задачи.
...
- **Редактирование клиента/проекта** — формы `client-edit-form`/`project-edit-form` (updateClient/
  updateProject) в `<details>` на страницах `[id]`; переименование и правка всех полей.
...
- **Изменения подтверждаются результатом БД**: Server Actions редактирования проекта и задачи
  возвращают обновлённую строку через `.select(...).maybeSingle()`. Пустой результат считается
  ошибкой доступа, а не успехом. Формы показывают явное сообщение об успешном сохранении.
  Дедлайн задачи можно быстро менять прямо в таблице «Задачи»; остальные поля остаются в общем
  боковом редакторе.
...
- **Жизненный цикл выполненной задачи** строится по `completed_at`: после отметки задача сразу
  уходит из активного списка, 30 дней видна во «Выполненных задачах», следующие 30 дней скрыта
  как архив, после 60 дней удаляется ежедневной задачей `pg_cron`. Триггер БД синхронизирует
  `completed_at` со статусом даже при изменениях вне UI.
